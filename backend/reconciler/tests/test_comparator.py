...
class ComparatorTestCase(unittest.TestCase):
    def test_detects_record_missing_in_system_b(self):
        ...
        self.assertEqual(results[0].record_id, 'REC-1015')
        self.assertEqual(results[0].org_id, 'ORG-A')
        self.assertEqual(results[0].val_a, '41095.33')
    ...
    def test_detects_orphan_record_in_system_b(self):
        ...
        self.assertEqual(results[0].record_id, 'REC-1999')
        self.assertEqual(results[0].org_id, 'ORG-A')
        self.assertIsNone(results[0].val_a)
        self.assertEqual(results[0].val_b, '41250.00')
    ...
    def test_detects_duplicate_entries_in_system_b(self):
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].reason, 'DUPLICATE_IN_SYSTEM_B')
        self.assertEqual(results[0].record_id, 'REC-1042')
        self.assertEqual(results[0].org_id, 'ORG-A')
        self.assertIn('112837.06; 112837.06', results[0].val_b)
    ...
    def test_detects_value_mismatch(self):
        ...
        self.assertEqual(results[0].record_id, 'REC-1003')
        self.assertEqual(results[0].org_id, 'ORG-B')
        self.assertEqual(results[0].val_a, '121388.01')
        self.assertEqual(results[0].val_b, '94834.38')
    ...
    def test_tenant_boundary_isolation(self):
        ...
        self.assertEqual(len(org_a_data), 1)
        self.assertEqual(org_a_data[0].record_id, 'REC-A01')
        self.assertTrue(all(d.org_id == 'ORG-A' for d in org_a_data))

        self.assertEqual(len(org_b_data), 1)
        self.assertEqual(org_b_data[0].record_id, 'REC-B01')
        self.assertTrue(all(d.org_id == 'ORG-B' for d in org_b_data))

    def test_canonical_reference_normalization_non_error(self):
        records_a = [
    ...
    def test_dirty_currency_and_comma_parsing(self):
        self.assertEqual(safe_parse_decimal('1,25,400.00'), Decimal('125400.00'))
        self.assertEqual(safe_parse_decimal('$120.50'), Decimal('120.50'))
        self.assertEqual(safe_parse_decimal('  450.00  '), Decimal('450.00'))
        self.assertIsNone(safe_parse_decimal(''))
        self.assertIsNone(safe_parse_decimal('N/A'))
        self.assertIsNone(safe_parse_decimal('NULL'))

    def test_blank_value_mismatch(self):
        records_a = [
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].reason, 'VALUE_MISMATCH')
        self.assertEqual(results[0].record_id, 'REC-1050')
        self.assertEqual(results[0].val_a, '160405.85')
        self.assertEqual(results[0].val_b, '')

    def test_real_dataset_reconciliation(self):
        data_dir = Path(__file__).resolve().parent.parent.parent.parent / 'data'
        self.assertTrue(data_dir.exists(), f"Data directory missing at {data_dir}")
        ...
        self.assertEqual(len(discrepancies), 10)
        ...
        self.assertEqual(len(org_a_discrepancies), 7)
        self.assertEqual(len(org_b_discrepancies), 3)

        self.assertFalse(any(d.record_id in {'REC-1112', '1112'} for d in discrepancies))
